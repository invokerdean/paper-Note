...
我们首先评估从阅读器天线到附着在用户身上的标签在不同距离处呼吸监测的准确性。在实验中，我们将天线的位置固定在地面以上1米，要求用户坐在距离天线1米到6米的不同位置。在每个实验中，用户根据在智能手机上运行的节拍器移动应用呼吸。呼吸频率范围从每分钟5到20次（bpm）。每个实验持续两分钟。我们连续测量呼吸信号并使用TagBreathe计算平均呼吸频率。我们重复实验100次。在实验过程中，我们共收集了大约50万个低水平的读数。

我们将TagBreathe的实验结果与ground truth进行比较，并绘制出图12中不同距离处的精确度。根据实验结果，1m处呼吸率测量的准确率为98.0％。虽然随着距离的增加准确度略有下降，但实验结果表明，整个实验的精度仍然保持在90.0％以上。随着通信范围的增加，读者观察到较弱的背向散射信号强度和较低的低电平数据读取速率，这对呼吸信号提取有负面影响。
###### 2)Impact of the number of users
我们评估TagBreathe与多个用户的表现。 用户并排距离天线4m。 每个用户佩戴三个商品无源标签。 商品阅读器从所有标签读取低等级数据，并且根据从标签ID提取的用户ID对数据进行分组。 我们用图13中的不同用户数量来评估监视精度。根据实验结果，不同用户数量的呼吸率精度保持在95.0％左右。 由于RFID防碰撞协议，来自不同用户的反向散射信号不会相互干扰。 此外，实验结果表明，即使4名使用12个标签的用户，商品读取器的读取率也足够高，以同时监测呼吸。
###### 3)Impact of contending tags
为了评估由于存在的标签数量增加而导致阅读率降低的影响，我们用RFID标签标记日常物品，并将RFID标签放置在商品阅读器的通信范围内。与连接到用户的呼吸监测标签相同，通信范围内的物品标签标签将根据标准EPC协议争用无线信道。结果，读取速率随着竞争标签的数量在通信范围内增加而降低。在实验中，用户佩戴3个标签，并坐在天线的前面。我们改变RFID标签的数量，并用不同的数字标签重复实验。商品读取器持续监控所有的标签，并读取其中的低级数据，包括用于标记日常物品的数据。在实验之前，我们覆盖3个呼吸监测标签的标签ID，以便读者识别它们并监测呼吸率。图14显示了存在的不同数量的竞争RFID标签的测量精度。根据实验结果，我们发现TagBreathe即使在通信范围内有30个竞争标签，也能达到91.0％的准确率。主要原因是因为总阅读率足够高，TagBreathe可以从呼吸监测标签收集足够的读数。当存在更多的竞争标签时，准确度降低，这导致3个呼吸监测标签的读取率降低。
###### 4)Impact of tag orientation and line-of-sight path.
 RFID系统的性能受天线方向的影响以及人体视线路径的阻塞。我们评估不同天线方向的监测精度。为了评估目的，我们只将一个定向天线连接到阅读器，以有意限制RFID系统的覆盖范围。在实际使用场景中，我们注意到可以连接多个天线来完全覆盖监控区域。如图15（a）所示，用户离天线4米，并以不同的方向重复实验。用户首先面向天线（即0°）并逆时针旋转直到用户面向相反的方向（即180°）。在图15（b）中，我们测量低层数据的读取速率以及不同方向的反向散射信号的RSSI。我们观察到，只要标签和天线之间存在视线路径（即[0°，90°]），反向散射信号的RSSI变化不大。另一方面，当用户旋转到90°时，读取速率从用户面对天线时的50赫兹降低到10赫兹。当用户进一步旋转（例如，[120°，180°]）时，由于视线路径路径被用户身体阻挡，阅读器不能识别标签或读取低级数据。在这种情况下（> 90°），TagBreathe不报告呼吸监测结果。
 
 在实际使用场景中，为了增加阅读器的覆盖范围，充分实现环境中的呼吸监测，商品阅读器可以连接多个天线，以确保实际上到达标签的视线路径。请注意，阅读器可以通过循环调度来协调多个天线，避免天线之间的干扰。而且，由于一次只有一个天线通电，并且RFID系统的整体功耗不会随着天线的数量而增加。
 
 图16示出了具有视线路径的不同标签方向（即，<90°）的测量精度。根据实验结果，当用户面对天线时，测量精度在90％以上。当用户旋转到90°时，准确度从90％降到85％。
###### 5)Impact of different postures
我们用不同的姿势，即坐着，站立和躺着来评估监视的准确性。 天线的位置固定在地面上方约1米处，标签与读取器之间的通信范围在整个实验中保持不变。 因此，结果主要受标签朝向天线的方向以及不同姿势的呼吸行为的影响。 根据实验结果，不同姿势的监测精度仍保持在90.0％以上
