...
## VI. EVALUATION
在本节中，我们将在实验室环境中对TagVision进行性能评估，如图7所示。
#### A.评估方法
在一个大小为400×800cm2的办公室里设计和执行三组实验。 我们通过附有标签的玩具火车模拟移动物体（如果需要的话），在不同形状（线性或弧形）的轨道上移动。 在激光水平的支持下，天线的位置由激光测距仪测量，误差为±0.1mm。 我们采用误差距离，定义为结果与地面实况之间的欧氏距离作为我们的基础度量。
#### B. 坐标转换准确度
作为CV子系统的核心模型，相机图像平面与物理世界之间的坐标转换精度直接影响到我们方法的最终结果。 如我们在§III-B中提到的那样，在校准摄像机的必要参数之后，我们在桌面上进行实验来检查准确性。 我们建立桌面的笛卡儿坐标系统，然后在桌面平面之间选取41个不同点作为地面真实点，并预先测量位置。 图8显示了坐标转换错误的CDF。 平均误差距离为x轴为0.96mm，y轴为1.05mm，组合尺寸为1.56mm，标准偏差为1.57mm。 此外，90％的误差小于2.74mm，最小误差为0.14mm，最大误差为7.61mm。 总的来说，坐标转换的精度非常高，为后面的工作奠定了基础。
#### C. 物体追踪精确度
为了达到更好的跟踪效果，TagVision采用密集光流法检测运动斑点，并结合均值漂移跟踪算法。 为了评估性能，我们降低了视频的帧率，并通过在每一帧手动标记玩具火车的中心位置来获得基本事实。 如图9所示，基本事实用蓝色标记为“+”，由CV子系统输出的跟踪结果用红色标记为“○”。 在20次重复实验的基础上，我们的跟踪方法的平均误差距离为x轴3.94mm，y轴3.25mm，组合尺寸为5.68mm，标准偏差为4.79mm，相对于 玩具火车的大小，约21×182 mm2。
#### D.轨迹匹配性能
1）匹配精度：扮演CV子系统和RFID子系统之间桥梁的角色，Blob匹配模块的性能对于TagVision是非常重要的。为了评估它，我们进行下面的实验。如图11所示，我们部署了四条轨道，每条轨道距离彼此大约20cm。RFID标签放置在轨道4的斑点处。我们运行我们的算法来计算每条轨道的匹配分数。图12分别显示了迭代1次和20次的归一化结果。地面实况是通过手动标记前面提到的贴有RFID标签的移动斑点来计算的。我们在每次迭代时从轨道上随机抽样30个位置。从图中可以清楚地看出，在这两种情况下，第4轨的匹配得分最高，这与实际情况是一致的，因为RFID标签确实在第四块。但是，只有1次迭代，Track 4的匹配分数与Track 3的匹配分数非常接近，与其他轨迹相比，不能够区分，而当迭代次数增加到20时，匹配分数变得更加稳定和容易分辨，甚至更多。我们的实验表明，20次迭代足以使得结果具有鲁棒性和判别性，并且只需要0.028s就完成了，所以我们将默认的迭代次数设置为20次。另外，由于我们的CV子系统具有相对较高的精度，轨道4非常接近地面真相。

此外，我们随不同形状（线性和非线性）改变从2到4的轨道数量，进行了比较研究。我们定义匹配准确率R如下：
R =# of successful matches/# of experiments in total × 100%
表II描述了获得的精确度比率以及轨道数量。很明显，所有的情况都有很高的准确率（平均为98％），差别不大（差距不到1％）。这证明了我们的融合算法的有效性。另外，随着轨道数量的增加，正确率也稍微提高。这是合理的，因为轨道越少，误差就会越少。 

2）采样数的影响：CV子系统的输出轨迹通常包含100多个位置，默认情况下我们随机采样30个。所以我们想知道采样点的数量是否会影响最终的斑点匹配的准确性。为了研究这个问题，我们把采样数从5到40间隔为5，对于每种情况，我们分别在2,3和4个轨道上进行测试。注意，我们采用默认的迭代次数为20.图10描述了匹配精度比率随采样数量的变化情况。我们有以下意见。 
1. 随着采样点数的增加，正确率也呈现出明显的上升趋势，从抽样数为5时的59％到抽样数达到40时的98％。抽样数少的原因因为融合算法的累积点过于有限，因此环境噪声引起的误差会对最终结果产生较大的影响，使目标跟踪难以区分。
2. 当采样数达到一定水平（≥30）时，匹配精度保持稳定在98％。这是因为算法本身成为我们充分采样的瓶颈。取样20点的精度已达94％。为了在计算成本和精度之间取得平衡，我们选择30作为默认采样数。
3. 在相同的采样数下，轨道数越少，精度就越高，这与我们在前一小节的结论是一致的。

#### E.标签跟踪的整体准确度
由于TagVision旨在跟踪标签对象，目标标签的最终跟踪精度是最关键的指标。 图13绘出跟踪误差的CDF。 我们观察到，大部分误差在10mm以内，因为标签与主机正确匹配，除了极少数100mm以外的异常值，这是由于斑点匹配故障造成的。 平均误差距离x轴为7.55mm，y轴为5.91mm，组合尺寸为10.33mm，标准偏差为21.23mm，90％误差小于12.13mm，这对于很多人来说已经足够好了应用。 融合算法的优异性能保证了标签对象的整体跟踪精度。
